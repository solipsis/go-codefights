/**
Challenge: Is Information Consistent
https://codefights.com/arcade/code-arcade/waterfall-of-integration/jkuhbNn7AdeCC7z7R/description
...
func isInformationConsistent(evidences [][]int) bool {
    ...
    // one column at a time, can't have 1 and -1
    for c := 0; c < len(evidences[0]); c++ {
        m := make(map[int]bool)
        for r := 0; r < len(evidences); r++ {            
            m[evidences[r][c]] = true 
        }
        if m[1] && m[-1] {
            return false
        }
